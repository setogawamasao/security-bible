...
<div id="window3" name="window" style="width:300px; height:300px;">
	...
	<div id="windowcontent3" class="funclistcontent">
       ...
       echo "<br>(graph not available in debug mode)";
		...
		<?php createFunctionList($user_functions_offset); ?>
		<div id="canvas3" style="display:none"></div>
		<canvas id="functioncanvas" tabindex="0" width="650" height="<?php echo (count(
      $user_functions_offset
  ) /
      4) *
      70 +
...
<div id="window4" name="window" style="width:300px; height:300px;">
	...
	<div id="windowcontent4" class="funclistcontent">
		<?php createUserinputList($user_input); ?>
	</div>
...
<div id="window5" name="window" style="width:300px; height:300px;">
	...
	<div id="windowcontent5" class="funclistcontent">
		...
		<?php createFileList($scanned_files, $file_sinks_count); ?>
		<div id="canvas5" style="display:none"></div>
		<canvas id="filecanvas" tabindex="0" width="650" height="<?php echo (count(
      $files
  ) /
      4) *
      70 +
...
if (empty($_POST["search"])) {
    ...
    if ($count_all > 0) {
        ...
        echo '<tr><td nowrap width="160" onmouseover="this.style.color=\'white\';" onmouseout="this.style.color=\'#DFDFDF\';" onClick="showAllCats()" style="cursor:pointer;" title="show all categories">Sum:</td><td>',
            $count_all,
            "</td></tr>";
    } else {
    ...
    echo '<tr><td colspan="2">',
        $count_matches == 0 ? "No" : $count_matches,
        " matches found.</td></tr>";
}

echo '</table><hr /><table class="textcolor" width="100%">',
    '<tr><td nowrap width="160" onmouseover="this.style.color=\'white\';" onmouseout="this.style.color=\'#DFDFDF\';" onClick="openWindow(5);eval(document.getElementById(\'filegraph_code\').innerHTML);maxWindow(5, 650);" style="cursor:pointer;" title="open files window">Scanned files:</td><td nowrap colspan="2">',
    count($files),
    "</td></tr>";
if (empty($_POST["search"])) {
    ...
    if ($count_inc > 0) {
        echo ($count_inc_success = $count_inc - $count_inc_fail) .
        "/" .
        $count_inc,
            " (" .
                ($round_inc_success =
    ...
    echo "</td></tr>",
        "<tr><td nowrap>Considered sinks:</td><td nowrap>",
        count($scan_functions),
        '</td><td rowspan="4" >';
    if (empty($_POST["search"]) && $count_all > 0) {
        echo '<div class="diagram"><canvas id="diagram" width="80" height="70"></canvas></div>';
    }
    echo "</td></tr>",
        '<tr><td nowrap onmouseover="this.style.color=\'white\';" onmouseout="this.style.color=\'#DFDFDF\';" onClick="openWindow(3);eval(document.getElementById(\'functiongraph_code\').innerHTML);maxWindow(3, 650);" style="cursor:pointer;" title="open functions window">User-defined functions:</td><td nowrap>' .
            (count($user_functions_offset) -
                (count($user_functions_offset) > 0 ? 1 : 0)) .
            "</td></tr>",
        '<tr><td nowrap onmouseover="this.style.color=\'white\';" onmouseout="this.style.color=\'#DFDFDF\';" onClick="openWindow(4);" style="cursor:pointer;" title="open userinput window">Unique sources:</td><td nowrap>' .
            count($user_input) .
            "</td></tr>",
        "<tr><td nowrap>Sensitive sinks:</td><td nowrap>" .
            (is_array($file_sinks_count) ? array_sum($file_sinks_count) : 0) .
            "</td></tr>",
        "</table><hr />";
    ...
    if (
        ...
        foreach ($info as $detail) {
            echo '<tr><td width="160">Info:</td><td><small>',
                $detail,
                "</small></td></tr>";
        }
        if (
...
?>
		<table class="textcolor" width="100%">
		<tr><td nowrap width="160">Scan time:</td><td nowrap><span id="scantime"><?php printf(
      "%.03f seconds",
      $elapsed
  ); ?></span></td></tr>
	</table>		
