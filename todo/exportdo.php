<?php
  ...
  if (!empty($query)) {
    ...
    $keys = array(':id' => $user->get_userid());
  }
  try {
    $dbh = dblogin();
    $sql = "SELECT todos.id, users.userid, todo, c_date, due_date, done, file, public FROM todos INNER JOIN users ON users.id=todos.owner AND ";
    if ($where !== '') {
      $sql .= $where;
    } else {
      $sql .= "users.userid=:id";
