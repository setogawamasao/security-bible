...
try {
    ...
    if (empty($name)) {
        $sql =
            "UPDATE todos SET todo=?, c_date=?, due_date=?, done=?, public=? WHERE id=?";
        $values = [$todo, $c_date, $due_date, $done, $public, $item];
    } else {
        $sql =
            "UPDATE todos SET todo=?, c_date=?, due_date=?, done=?, file=?, public=? WHERE id=?";
        $values = [$todo, $c_date, $due_date, $done, $name, $public, $item];
    }
    $sth = $dbh->prepare($sql);
