...
try {
    $dbh = dblogin();
    $sql =
        "SELECT todos.id, users.userid, users.icon, todo, c_date, due_date, done, file, public FROM todos INNER JOIN users ON todos.id = ? AND users.id = todos.owner AND (todos.owner = ? OR todos.public > 0 OR ? > 0)";

    $sth = $dbh->prepare($sql);
    $sth->execute([$item, $id, $user->is_super()]);
    $result = $sth->fetch();
...
<?php require "menu.php"; ?>
  <div id="contents">
    <?php if (!empty($result)): ?>
      ...
      <td>ID</td><td><?php e(
          $result["userid"]
      ); ?><img src="resize.php?path=icons&basename=<?php e(
    $result["icon"]
); ?>&size=64"></td>
      </tr>
      <tr>
